...
------------------------------------------------ Link for the challenge: https://codeforces.com/problemset/problem/181/A ------------------------------------------------
...
Deduction tells Polycarpus that the fourth robbery will be committed in such district, that all four robbed districts will form the vertices of some rectangle, parallel to the sides of the map.
...
for i in range(n):
    ...
    aestrics = temp.count('*')
    if(aestrics == 2):
        pos = [i for i, letter in enumerate(temp) if letter == '*']
        positions.append(pos[0] + 1)
        positions.append(pos[1] + 1)
    elif(aestrics == 1):
        positionThird = temp.index('*') + 1
        positions.append(positionThird)
        row = i + 1
