...
----------------------------------------------------------- Link for the challenge: https://codeforces.com/problemset/problem/1305/A -----------------------------------------------------------
...
Kuroni wants to give exactly one necklace and exactly one bracelet to each of his daughters. 
To make sure that all of them look unique, the total brightnesses of the gifts given to each daughter should be pairwise distinct. 
Formally, if the i-th daughter receives a necklace with brightness xi and a bracelet with brightness yi, then the sums xi+yi should be pairwise distinct. Help Kuroni to distribute the gifts.
...
Output
For each test case, print a line containing n integers x1,x2,…,xn, representing that the i-th daughter receives a necklace with brightness xi. 
In the next line print n integers y1,y2,…,yn, representing that the i-th daughter receives a bracelet with brightness yi.
...
for _ in range(cases):
    sze = int(input())
    necklaces = sorted(list(map(int, input().split())))
    bracelets = sorted(list(map(int, input().split())))
    print(*necklaces)
    print(*bracelets)